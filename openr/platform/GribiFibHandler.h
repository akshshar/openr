...
#include <openr/gribi/GribiRshuttle.h>

namespace openr {

extern const uint8_t kAqRouteProtoId;

using GribiServer = std::unordered_set<std::pair<std::string, std::string>>;
 ...
 */
class GribiFibHandler final : public thrift::FibServiceSvIf {
 public:
  explicit GribiFibHandler(fbzmq::ZmqEventLoop* zmqEventLoop,
                             std::vector<VrfData> vrfSet,
                             std::shared_ptr<grpc::Channel> Channel);
  ~GribiFibHandler() override {}
...
//  folly::Future<std::unique_ptr<std::vector<openr::thrift::UnicastRoute>>>
//  future_getGribiRouteTable() override;

 private:
  GribiFibHandler(const GribiFibHandler&) = delete;
  GribiFibHandler& operator=(const GribiFibHandler&) = delete;

  // Used to interact with the IOS-XR routing table
  std::unique_ptr<GribiRshuttle> gribiRshuttle_;
