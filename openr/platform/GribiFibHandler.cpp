...
void
GribiFibHandler::setVrfContext(std::string vrfName)
{
    // The VRF context gets set for all following operations
    gribiRshuttle_->setGribiRouteVrf(vrfName);
}


GribiFibHandler::GribiFibHandler(fbzmq::ZmqEventLoop* zmqEventLoop,
  ...
  :startTime_(std::chrono::duration_cast<std::chrono::seconds>(
                     ...
                     .count())
{
   LOG(INFO) << "GribiFibhandler";

   gribiRshuttle_ = std::make_unique<GribiRshuttle>(zmqEventLoop,
                                        vrfSet,
                                        kAqRouteProtoId,
                                        Channel);
...
GribiFibHandler::future_addUnicastRoute(
  ...
  return gribiRshuttle_->addUnicastRoute(prefix, newNextHops);
}

folly::Future<folly::Unit>
GribiFibHandler::future_deleteUnicastRoute(
  ...
  return gribiRshuttle_->deleteUnicastRoute(myPrefix);
}

folly::Future<folly::Unit>
GribiFibHandler::future_addUnicastRoutes(
    int16_t clientId,
    ...
    for (auto& route : *routes) {
        LOG(INFO) << "Got prefix ..." << toString(route.dest) ;
...
folly::Future<folly::Unit>
GribiFibHandler::future_deleteUnicastRoutes(
...
folly::Future<folly::Unit>
GribiFibHandler::future_syncFib(
    int16_t clientId,
    std::unique_ptr<std::vector<thrift::UnicastRoute>> routes) {
    ...
    return gribiRshuttle_->syncRoutes(newRouteDb);
}

folly::Future<int64_t>
GribiFibHandler::future_periodicKeepAlive(int16_t /* clientId */) {

    LOG(INFO) << "Received KeepAlive from OpenR";

    return folly::makeFuture(keepAliveId_++);
}

int64_t
GribiFibHandler::aliveSince() {
  return startTime_;
}

folly::Future<std::unique_ptr<std::vector<openr::thrift::UnicastRoute>>>
GribiFibHandler::future_getRouteTableByClient(int16_t clientId) {
  LOG(INFO) << "Get routes from FIB for clientId " << clientId;

  return gribiRshuttle_->getUnicastRoutes()
...
void
GribiFibHandler::getCounters(std::map<std::string, int64_t>& counters) {
  LOG(INFO) << "Get counters requested";
  auto routes = gribiRshuttle_->getUnicastRoutes().get();
